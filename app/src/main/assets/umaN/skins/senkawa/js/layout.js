		...
		$cards.each(function () {
			...
			$.ajax({
				...
				[].map.call(data.data.articles, function (article) {
					$table.append(
						'<a href="//namu.live/b/' + slug + '/' + article.id + '" data-npjax="true">' +
							article.title +
						"</a>"
					);
				});
		...
		$.ajax({
			...
			if (data.type === "recent" || data.type === "hot") {
				$topbarArea.html(
					...
					'<a href="//namu.live' + data.link + '" data-npjax="true">[' + data.board + '] ' + data.title.substr(0,100) + "</a>"
				);
			} else if (data.type === "empty") {
				$topbarArea.html(
					'<span class="leaf-info">나무라이브</span>&nbsp;' +
					'<a href="//namu.live/b/' + data.slug + '/write" data-npjax="true">\'' + data.board + '\'에 인기글이 아직 없습니다. 인기글에 도전하세요!</a>'
				);
			}
